 ...
 *
 * <p>
 * This component accepts a list of customers determined to be unsynced,
 * groups them by country, computes and persists a stable row-hash for each
 * customer, renders CSV files per country and uploads those files to S3.
 * The CSV uploads are performed in a transaction hook so that uploaded
 ...
 * Notes:
 * <ul>
 * <li>Method {@link #syncAndUpload(List)} is {@link Transactional} and
 * registers a {@link TransactionSynchronization} to perform S3
 * uploads in {@code beforeCommit} and cleanup in {@code afterCompletion}.</li>
 * <li>The service persists {@link SyncedCustomerHash} entries (row hashes)
 * for each processed customer before attempting S3 uploads.</li>
 * </ul>
 * </p>
 */
@Slf4j
...
public class CustomerSyncService {
     ...
     * Behavior:
     * <ol>
     * <li>Group {@code unsyncedCustomers} by {@link Customer#getLand()}.</li>
     * <li>For each customer compute a stable row-hash and persist a
     * {@link SyncedCustomerHash} entry.</li>
     * <li>Render a CSV for each country and schedule S3 uploads inside a
     * transaction synchronization. Uploads happen in
     * {@code beforeCommit} so they only occur when the transaction
     * successfully commits; uploaded files are deleted in
     * {@code afterCompletion} if the transaction rolled back.</li>
     * </ol>
     * </p>
     *
    ...
    public void syncAndUpload(List<Customer> unsyncedCustomers) {
        ...
        Map<String, List<Customer>> byCountry = unsyncedCustomers.stream()
        ...
        for (Map.Entry<String, List<Customer>> entry : byCountry.entrySet()) {
            ...
            for (Customer customer : customers) {
                String rowHash = hashService.customerRowHash(
                ...
                syncedCustomerHashRepository.save(
                        SyncedCustomerHash.builder()
                                .kundenId(customer.getId())
                                .rowHash(rowHash)
                                .build());
            }
            ...
            String filename = "kunde_" + country + "_"
