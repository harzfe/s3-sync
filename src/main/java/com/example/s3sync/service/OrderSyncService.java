 ...
 *
 * <p>
 * This component accepts a list of orders determined to be unsynced,
 * groups them by country (resolved via the related customer's country),
 * computes and persists a marker/hash for each order, renders CSV files
 * per country and uploads those files to S3. Uploads are scheduled in a
 * transaction synchronization so that objects are cleaned up if the
 ...
 * Notes:
 * <ul>
 * <li>The main method {@link #syncAndUpload(List)} is annotated with
 * {@link Transactional} and registers a {@link TransactionSynchronization}
 * to perform uploads in {@code beforeCommit} and cleanup on rollback.</li>
 * <li>The service persists {@link SyncedOrderHash} entries (marker hashes)
 * for each processed order prior to scheduling uploads.</li>
 * </ul>
 * </p>
 */
@Slf4j
...
public class OrderSyncService {
     ...
     * Behavior:
     * <ol>
     * <li>Group {@code unsyncedOrders} by country, resolving the country via
     * the related customer.</li>
     * <li>For each order compute a stable marker/hash and persist a
     * {@link SyncedOrderHash} entry.</li>
    ...
    public void syncAndUpload(List<Order> unsyncedOrders) {
        ...
        Map<String, List<Order>> byCountry = unsyncedOrders.stream()
        ...
        for (Map.Entry<String, List<Order>> entry : byCountry.entrySet()) {
            ...
            String filename = "auftraege_" + country + "_"
