...
class CustomerSyncServiceUnitTest {
    ...
    @Mock
    SyncedCustomerHashRepository syncedCustomerHashRepository;
    @Mock
    ...
    /**
     * Persist computed row-hashes for each unsynced customer and save a
     * corresponding tracking entry per customer.
     */
    @Test
    void persistsUnsyncedCustomers() {
        ...
        when(hashService.customerRowHash(any(), any(), any(), any(), any(), any(), any(), any(), any(), any()))
                .thenReturn("hash1", "hash2", "hash3");

        service.syncAndUpload(customers);

        verify(hashService, times(3)).customerRowHash(any(), any(), any(), any(), any(), any(), any(), any(), any(),
                any());
        ...
        assertThat(saved.getAllValues())
                ...
                .containsExactlyInAnyOrder(
                        Tuple.tuple(1L, "hash1"),
                        Tuple.tuple(2L, "hash2"),
                        Tuple.tuple(3L, "hash3"));
    }

    /**
     * Ensure customers are grouped by their country code so that a separate
     * CSV is created/uploaded per country.
     */
    @Test
