...
public class CsvServiceTest {
        ...
        /**
         * Verify that a list of {@link CustomerCsvDto} is rendered to CSV with the
         * expected column order and UTF-8 encoding. The test asserts the exact
         * output.
         */
        @Test
        void CustomerCsvDto_writesToCsv_inExpectedFormat() {
                List<CustomerCsvDto> customerCsvDtos = List.of(CustomerCsvDto.builder()
        ...
        /**
         * Verify that a list of {@link OrderCsvDto} is rendered to CSV with the
         * expected column order (auftragId, artikelnummer, kundeId).
         */
        @Test
        void orderCsvDto_writesToCsv_inExpectedFormat() {
                var rows = List.of(OrderCsvDto.builder()
                                .auftragId("1")
                                .artikelnummer("123")
                                .kundeId("1")
                                .build(),
                ...
                byte[] bytes = csvService.ordersToCsv(rows);
                String csv = new String(bytes, StandardCharsets.UTF_8);
