...
public class OrderSyncServiceTest {
    ...
    @Mock
    SyncedOrderHashRepository syncedOrderHashRepository;
    @Mock
    ...
    /**
     * Persist marker hashes for each unsynced order and save a corresponding
     * tracking entry per order.
     */
    @Test
    void persistsUnsyncedOrders() {
        ...
        when(hashService.orderMarkerHash(anyString()))
    ...
    /**
     * Ensure orders are grouped by the customer country (via getLand) so that
     * separate CSVs are produced per country.
     */
    @Test
