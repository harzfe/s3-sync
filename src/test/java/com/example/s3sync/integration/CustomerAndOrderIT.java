...
public class CustomerAndOrderIT extends BaseIT {
  ...
  /**
   * Integration test that verifies the normal flow where both a customer and an order exist: both
   * customer and order CSVs should be uploaded. A subsequent immediate run must not change the
   * uploaded objects (ETag stays the same) since nothing changed.
   */
  @Test
  void runITFlow_customer_and_order() {
    ...
    assertThat(heeadObjectResponseOrderSecondRun.eTag())
        .isEqualTo(heeadObjectResponseOrderFirstRun.eTag());
  }
}
